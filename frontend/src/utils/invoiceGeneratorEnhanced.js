...
/**
 * Enhanced Invoice Generator with improved error handling and styling
...
export const generateInvoicePDF = (booking) => {
  try {
    ...
    if (typeof doc.autoTable !== 'function') {
      ...
      throw new Error('PDF table plugin not available. Please refresh the page.');
    }
    ...
    // const silverColor = [192, 192, 192]; // #c0c0c0 - Silver (reserved for future use)
...
/**
 * Quick test function to validate invoice generation
 */
export const testInvoiceGenerator = () => {
  const testBooking = {
    bookingReference: 'BK-TEST-12345',
    status: 'completed',
    rideType: 'hourly',
    pickupLocation: { address: '123 Main Street, New York, NY 10001' },
    dropoffLocation: { address: '456 Park Avenue, New York, NY 10022' },
  ...
  try {
    generateInvoicePDF(testBooking);
